...
import { SupplyBiz_YELLOW, generateId } from '../utils/simulationUtils';
  ...
  return (
    <group>
      ...
      >
        <Box args={[8, 0.02, 8]} position={[0, 0.06, 0]}>
          <meshStandardMaterial 
            color={selectedZone === 'zoneA' ? SupplyBiz_YELLOW : '#34495e'} 
            opacity={0.7} 
            transparent 
          />
        </Box>
        ...
        {inventoryData.zoneA.items.slice(0, 20).map((item, i) => (
          <Float key={item.id} speed={1} rotationIntensity={0.1} floatIntensity={0.1}>
            <Box args={[0.4, 0.4, 0.4]} position={item.position}>
              <meshStandardMaterial color={SupplyBiz_YELLOW} />
            </Box>
          </Float>
        ))}
      </group>
      ...
      >
        <Box args={[8, 0.02, 8]} position={[0, 0.06, 0]}>
          <meshStandardMaterial 
            color={selectedZone === 'zoneB' ? SupplyBiz_YELLOW : '#34495e'} 
            opacity={0.7} 
            transparent 
          />
        </Box>
      ...
      >
        <Box args={[8, 0.02, 8]} position={[0, 0.06, 0]}>
          <meshStandardMaterial 
            color={selectedZone === 'zoneC' ? SupplyBiz_YELLOW : '#34495e'} 
            opacity={0.7} 
            transparent 
          />
        </Box>
      ...
      <group position={[0, 1, 2]}>
        ...
        {[0, 2, 4, 6, 8].map((x, i) => (
          <Box key={i} args={[0.6, 0.4, 0.6]} position={[x - 4, 0.3, 0]}>
            <meshStandardMaterial color={i % 2 === 0 ? SupplyBiz_YELLOW : '#3498db'} />
          </Box>
        ))}
      </group>
